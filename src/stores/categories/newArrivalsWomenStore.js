...
export const useNewArrivalsWomenStore = defineStore(
  "new-arrivals-women-store",
  {
    state: () => {
      return {
        images: [],
    ...
    actions: {
      async fetchAllImages() {
        try {
          const imagesRef = ref(storage, "/New Arrivals (women)");
          ...
          const querySnap = await getDocs(
            query(collection(db, "new-arrivals-women"))
          );
          let products = [];
      ...
      async uploadImg(file) {
        ...
        const storagePath = "/New Arrivals (women)/" + file.name;
