...
export const useKidsGlobalDealsStore = defineStore("kids-global-products", {
  state: () => {
    return {
      images: [],
      files: [],
  ...
  actions: {
    async fetchAllImages() {
      try {
        const imagesRef = ref(storage, "/Global Deals (kids)");
        ...
        const querySnap = await getDocs(
          query(collection(db, "kids-global-deals"))
        );
        let products = [];
    ...
    async uploadImg(file) {
      ...
      const storagePath = "/Global Deals (kids)/" + file.name;
