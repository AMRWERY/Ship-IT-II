...
export const useWomenGlobalDealsStore = defineStore("women-global-products", {
  state: () => {
    return {
      images: [],
      files: [],
  ...
  actions: {
    async fetchAllImages() {
      try {
        const imagesRef = ref(storage, "/Global Deals (women)");
        ...
        const querySnap = await getDocs(
          query(collection(db, "women-global-deals"))
        );
        let products = [];
    ...
    async uploadImg(file) {
      ...
      const storagePath = "/Global Deals (women)/" + file.name;
