...
export const useNewArrivalsMenStore = defineStore("new-arrivals-men-store", {
  state: () => {
    return {
      images: [],
      files: [],
  ...
  actions: {
    async fetchAllImages() {
      try {
        const imagesRef = ref(storage, "/New Arrivals (men)");
        ...
        const querySnap = await getDocs(
          query(collection(db, "new-arrivals-men"))
        );
        let products = [];
    ...
    async uploadImg(file) {
      ...
      const storagePath = "/New Arrivals (men)/" + file.name;
