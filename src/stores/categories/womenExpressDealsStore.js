...
export const useWomenExpressDealsStore = defineStore("women-express-products", {
  state: () => {
    return {
      images: [],
      files: [],
  ...
  actions: {
    async fetchAllExpressImages() {
      try {
        const imagesRef = ref(storage, "/Express Deals (women)");
        ...
        const querySnap = await getDocs(
          query(collection(db, "women-express-deals"))
        );
        let products = [];
    ...
    async uploadImg(file) {
      ...
      const storagePath = "/Express Deals (women)/" + file.name;
