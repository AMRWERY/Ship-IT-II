...
export const useKidsExpressDealsStore = defineStore("kids-express-products", {
  state: () => {
    return {
      images: [],
      files: [],
  ...
  actions: {
    async fetchAllExpressImages() {
      try {
        const imagesRef = ref(storage, "/Express Deals (kids)");
        ...
        const querySnap = await getDocs(
          query(collection(db, "kids-express-deals"))
        );
        let products = [];
    ...
    async uploadImg(file) {
      ...
      const storagePath = "/Express Deals (kids)/" + file.name;
